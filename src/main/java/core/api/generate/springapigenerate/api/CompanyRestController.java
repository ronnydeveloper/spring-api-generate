...
public class CompanyRestController {
    ...
    public ResponseEntity<ApiResponse> list() {
        ...
        ApiResponse<List<Company>> apiResponse = this.companyService.doView();
    ...
    @PostMapping("/preview")
    @PreAuthorize("@securityConfigService.hasPermission()")
    public ResponseEntity<ApiResponse> preview(@RequestBody Company company) {
        logger.info("Call preview function");
        ApiResponse apiResponse = this.companyService.doPreview(company);
        return ResponseEntity.status(HttpStatus.OK).contentType(MediaType.APPLICATION_JSON).body(apiResponse);
    }
