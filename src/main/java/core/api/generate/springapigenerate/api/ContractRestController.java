...
public class ContractRestController {
    ...
    public ResponseEntity<ApiResponse> list() {
        ...
        ApiResponse<List<Contract>> apiResponse = this.contractService.doView();
    ...
    @PostMapping("/preview")
    @PreAuthorize("@securityConfigService.hasPermission()")
    public ResponseEntity<ApiResponse> preview(@RequestBody Contract contract) {
        logger.info("Call preview function");
        ApiResponse apiResponse = this.contractService.doPreview(contract);
        return ResponseEntity.status(HttpStatus.OK).contentType(MediaType.APPLICATION_JSON).body(apiResponse);
    }
