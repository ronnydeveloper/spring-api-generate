...
public class CurrencyRateRestController {
    ...
    public ResponseEntity<ApiResponse> list() {
        ...
        ApiResponse<List<CurrencyRate>> apiResponse = this.currencyRateService.doView();
    ...
    @PostMapping("/preview")
    @PreAuthorize("@securityConfigService.hasPermission()")
    public ResponseEntity<ApiResponse> preview(@RequestBody CurrencyRate currencyRate) {
        logger.info("Call preview function");
        ApiResponse apiResponse = this.currencyRateService.doPreview(currencyRate);
        return ResponseEntity.status(HttpStatus.OK).contentType(MediaType.APPLICATION_JSON).body(apiResponse);
    }
