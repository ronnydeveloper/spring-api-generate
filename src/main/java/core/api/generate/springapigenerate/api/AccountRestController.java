...
public class AccountRestController {
    ...
    public ResponseEntity<ApiResponse> list() {
        ...
        ApiResponse<List<Account>> apiResponse = this.accountService.doView();
    ...
    @PostMapping("/preview")
    @PreAuthorize("@securityConfigService.hasPermission()")
    public ResponseEntity<ApiResponse> preview(@RequestBody Account account) {
        logger.info("Call preview function");
        ApiResponse apiResponse = this.accountService.doPreview(account);
        return ResponseEntity.status(HttpStatus.OK).contentType(MediaType.APPLICATION_JSON).body(apiResponse);
    }
