...
public class StaticDataRestController {
    ...
    public ResponseEntity<ApiResponse> list() {
        ...
        ApiResponse<List<StaticData>> apiResponse = this.staticDataService.doView();
    ...
    @PostMapping("/preview")
    @PreAuthorize("@securityConfigService.hasPermission()")
    public ResponseEntity<ApiResponse> preview(@RequestBody StaticData staticData) {
        logger.info("Call preview function");
        ApiResponse apiResponse = this.staticDataService.doPreview(staticData);
        return ResponseEntity.status(HttpStatus.OK).contentType(MediaType.APPLICATION_JSON).body(apiResponse);
    }
