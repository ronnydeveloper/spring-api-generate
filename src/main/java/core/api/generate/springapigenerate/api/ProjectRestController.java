...
public class ProjectRestController {
    ...
    public ResponseEntity<ApiResponse> list() {
        ...
        ApiResponse<List<Project>> apiResponse = this.projectService.doView();
    ...
    @PostMapping("/preview")
    @PreAuthorize("@securityConfigService.hasPermission()")
    public ResponseEntity<ApiResponse> preview(@RequestBody Project project) {
        logger.info("Call preview function");
        ApiResponse apiResponse = this.projectService.doPreview(project);
        return ResponseEntity.status(HttpStatus.OK).contentType(MediaType.APPLICATION_JSON).body(apiResponse);
    }
